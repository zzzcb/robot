...
int main(int argc, char **argv) {
    ...
    spinner.start();
    ros::NodeHandle node_handle("motion_planning_api_tutorial");
    //2 setup
    //Planners are setup as plugins in MoveIt and you can use the ROS pluginlib interface to load any planner that you want to use.
    ...
    planning_scene->getCurrentStateNonConst().setToDefaultValues(joint_model_group,
                                                                 "ready"); //  name("ready") defined in the SRDF group_state tag
    ...
    // We will get the name of planning plugin we want to load from the ROS parameter server,
    // and then load the planner making sure to catch all exceptions.
    ...
    visual_tools.publishText(text_pose, "Motion Planning API Demo by zcb", rvt::WHITE, rvt::XLARGE);
    ...
    //6 Now, we go back to the first goal to prepare for orientation constrained planning
    req.goal_constraints.clear();
    req.goal_constraints.push_back(pose_goal);
    context = planner_instance->getPlanningContext(planning_scene, req, res.error_code_);
    context->solve(res);
