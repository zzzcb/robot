...
#include <kdl/chainiksolverpos_nr_jl.hpp> // jl :是 joint limit
...
int main() {
    ...
    //  反解 nr_jl
    // joint limit
    JntArray min_angles(6);
    min_angles(0) = -360 * PI / 180;
    min_angles(1) = -360 * PI / 180;
    min_angles(2) = -360 * PI / 180;
    min_angles(3) = -360 * PI / 180;
    min_angles(4) = -360 * PI / 180;
    min_angles(5) = -360 * PI / 180;

    JntArray max_angles(6);
    max_angles(0) = 360 * PI / 180;
    max_angles(1) = 360 * PI / 180;
    max_angles(2) = 360 * PI / 180;
    max_angles(3) = 360 * PI / 180;
    max_angles(4) = 360 * PI / 180;
    max_angles(5) = 360 * PI / 180;

    ChainFkSolverPos_recursive fk_solver(chain);
    ChainIkSolverVel_pinv ik_pinv(chain);

    ChainIkSolverPos_NR_JL ik_solver(chain, min_angles, max_angles, fk_solver, ik_pinv);
