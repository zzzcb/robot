...
void findCorners(cv::Mat &src, cv::Size &patternSize, bool &isFound, vector<cv::Point2f> &corners) {
    ...
    isFound = cv::findCirclesGrid(gray, patternSize, corners, cv::CALIB_CB_ASYMMETRIC_GRID);// 非对称圆网格
}
...
void getPoints(cv::Size &patternSize, float square_size, vector<cv::Point3f> &points) {
     ...
     *  [0,2,0]         [2,2,0]         [4,2,0]
     */
    for (int i = 0; i < patternSize.height; ++i) { // 不用 * 2
        for (int j = 0; j < patternSize.width; ++j) {
            points.push_back(cv::Point3f((j * 2 + i % 2) * square_size, i * square_size, 0));
        }
    }
}
...
int main() {
    cv::Size patternSize(4, 11); // 圆形网格的 数量
    ...
    if (!capture.isOpened()) {
        ...
        //6 保存标定数据到 文件
        saveToFile("/home/zcb/camera_calibration/01calibration/04_calibration_a_circleboard_video.xml", cameraMatrix,
                   distCoeffs, r_mse);
        saveToFile("/home/zcb/camera_calibration/01calibration/04_calibration_a_circleboard_video.yml", cameraMatrix,
                   distCoeffs, r_mse);
