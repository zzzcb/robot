...
int main(int argc, char **argv) {
    ...
    cv::glob("/home/zcb/02gazebo_camera_calibration/src/camera_in_cali_pkg/images/pic*.png", paths);
    int idx = 0;
    cv::Size imgSize;
    for (auto path:paths) {
...
//    }
    ...
    if (cv::checkRange(cameraInMatrix) && cv::checkRange(distCoeffs)) {
        saveToFile("/home/zcb/02gazebo_camera_calibration/src/camera_in_cali_pkg/calibration_result/camera_in.xml",
                   cameraInMatrix, distCoeffs,
                   r_mse, imgSize);
        saveToFile("/home/zcb/02gazebo_camera_calibration/src/camera_in_cali_pkg/calibration_result/camera_in.yml",
                   cameraInMatrix, distCoeffs,
                   r_mse, imgSize);
    } else {
        cout << "标定失败" << endl;
